...
_RL_DATA_DIR = flags.DEFINE_string(
    'rl_data_dir', None, 'Directory to save reinforcement learning data.'
)
...
def run_alphageometry(
  ...
  if run_ddar(g, p, out_file):
    if _COLLECT_RL_DATA.value and rl_collector:
      # collect the RL data for the successful proof.
      rl_collector.add_search_result(
          p.url, p.txt(), [(string, 0.0)], [], limit_per_problem=1
      )
    return True
